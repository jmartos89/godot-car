...
func get_speed_kph():
	var speed = current_speed_mps * 3600.0 / 1000.0
	if (speed < 1):
		speed = 0
	return speed
...
func _ready():
	$BrakeLightLeft.visible = false
	$BrakeLightRight.visible = false
...
func _physics_process(delta):
	...
	if (accelerate):
		if (!$AudioAccelerate.playing):
			$AudioAccelerate.play()
		else:
			if ($AudioAccelerate.get_playback_position() > 3.5):
				$AudioAccelerate.seek(1)
	else:
		if (stop):
			if ($AudioAccelerate.get_playback_position() < 3.5):
				$AudioAccelerate.seek(3.5)
		else:
			if ($AudioAccelerate.get_playback_position() < 2.5):
				$AudioAccelerate.seek(2.5)
				
	
	if (stop):
		$BrakeLightLeft.visible = true
		$BrakeLightRight.visible = true
	else:
		$BrakeLightLeft.visible = false
		$BrakeLightRight.visible = false
